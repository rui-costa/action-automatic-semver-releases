...
MAJOR=`echo $LATEST | tail -1 | cut -d '.' -f 1`
fi
...
if [ $semver = 'MAJOR' ]
then
  MINOR=`echo $LATEST | tail -1 | cut -d '.' -f 2 | awk '{$1=$1+1;1}'`
else
  MINOR=`echo $LATEST | tail -1 | cut -d '.' -f 2`
fi
...
if [ $semver = 'PATCH' ]
then
  PATCH=`echo $LATEST | tail -1 | cut -d '.' -f 3 | awk '{$1=$1+1;1}'`
else
  PATCH=`echo $LATEST | tail -1 | cut -d '.' -f 3`
fi
